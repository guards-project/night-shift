...
public:
    ...
    */
     ...
     */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, AdvancedDisplay, Category = Dungeon)
    TArray<TObjectPtr<USnapGridFlowModuleSelectionRule>> ModuleResolveRules;
};

UCLASS(Blueprintable)
class DUNGEONARCHITECTRUNTIME_API USnapGridFlowModel : public USnapDungeonModelBase {
    GENERATED_BODY()
public:
    UPROPERTY()
...
UCLASS(Meta=(DisplayName="Snap Grid Flow", Description="Stitch pre-built snap modules using a 3D grid flow graph. Create multi-floor dungeons, side scrollers, top-down dungeon crawlers and much more with this powerful builder"))
