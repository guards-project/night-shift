...
public:
    ...
    TArray<FGuid> GetOutgoingNodes(const FGuid& InNodeId) const {
        const TArray<FGuid>* SearchResult = OutgoingNodes.Find(InNodeId);
        return SearchResult ? *SearchResult : TArray<FGuid>();
    }
    
    TArray<FGuid> GetIncomingNodes(const FGuid& InNodeId) const {
        const TArray<FGuid>* SearchResult = IncomingNodes.Find(InNodeId);
        return SearchResult ? *SearchResult : TArray<FGuid>();
    }

    TArray<FGuid> GetConnectedNodes(const FGuid& InNodeId) const {
        const TArray<FGuid>* SearchResult = ConnectedNodes.Find(InNodeId);
        return SearchResult ? *SearchResult : TArray<FGuid>();
    }
    
    TArray<FGuid> GetConnectedLinks(const FGuid& InNodeId) const {
        const TArray<FGuid>* SearchResult = ConnectedLinks.Find(InNodeId);
        return SearchResult ? *SearchResult : TArray<FGuid>();
    }
