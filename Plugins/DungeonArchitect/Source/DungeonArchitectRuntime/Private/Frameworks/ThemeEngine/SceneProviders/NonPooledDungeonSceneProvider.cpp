...
void FNonPooledDungeonSceneProvider::AddStaticMesh(UStaticMesh* Mesh, const FDungeonSceneProviderContext& Context) {
    if (!Mesh) return;

    const TSharedPtr<FSceneProviderCommand> Command = MakeShareable(
        new SceneProviderCommand_CreateMeshSimple(Dungeon, LevelOverride, Context, Mesh));
    GameThreadCommands.Add(Command);
}
