...
void FPooledDungeonSceneProvider::AddStaticMesh(UStaticMesh* Mesh, const FDungeonSceneProviderContext& Context) {
    if (!Mesh) return;
    ...
    // Check if we have a free actor of this type in the pool. If so use it.
    if (AActor* Actor = ReuseFromPool(Context.NodeId, Context.Transform)) {
        Command = MakeShareable(new SceneProviderCommand_ReuseStaticMeshSimple(Dungeon, Context, Actor, Mesh));
    }
    else {
        // No free actor exists.  Create a new actor later in the game thread
        Command = MakeShareable(new SceneProviderCommand_CreateMeshSimple(Dungeon, LevelOverride, Context, Mesh));
    }
    GameThreadCommands.Add(Command);
}
