...
void FSnapThemeSceneProvider::AddStaticMesh(UStaticMesh* Mesh, const FDungeonSceneProviderContext& Context) {
    SceneProviderCommand_CreateMeshSimple Command(Dungeon, LevelOverride, Context, Mesh);
    Command.GetOnActorSpawned().BindRaw(this, &FSnapThemeSceneProvider::OnActorSpawned);
    Command.Execute(GetDungeonWorld());
}
