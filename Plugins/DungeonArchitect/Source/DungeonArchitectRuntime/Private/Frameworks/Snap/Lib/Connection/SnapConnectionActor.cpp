        ...
        : Super(ObjectInitializer)
{
    ...
    if (!IsRunningCommandlet()) {
        ...
        struct FConstructorStatics {
            ...
            ConstructorHelpers::FObjectFinderOptional<UTexture2D> IconRoomEntry;
            ConstructorHelpers::FObjectFinderOptional<UTexture2D> IconRoomExit;
            ...
            FConstructorStatics()
                ...
                , IconRoomEntry(TEXT("/DungeonArchitect/Core/Textures/icon_ConnRoomEntry"))
                , IconRoomExit(TEXT("/DungeonArchitect/Core/Textures/icon_ConnRoomExit"))
        ...
        */
        ...
        IconConstraintRoomEntry = ConstructorStatics.IconRoomEntry.Get();
        IconConstraintRoomExit = ConstructorStatics.IconRoomExit.Get();
...
UTexture2D* ASnapConnectionActor::GetConstraintTexture(ESnapConnectionConstraint ConnectionConstraint) {
    ...
    case ESnapConnectionConstraint::RoomEntry:
        return IconConstraintRoomEntry;

    case ESnapConnectionConstraint::RoomExit:
        return IconConstraintRoomExit;
...
void ASnapConnectionActor::BuildImpl(int32 InSeed, ADungeon* InDungeon, const FGuid& OwningModuleId, const FGuid& OtherModuleId, ULevel* HostLevel) {
    ...
    MarkerName = ConnectionComponent->MarkerPrefix + MarkerName;
    ...
    FSnapConnectionSelectorInfo ModuleInfoA;
    FSnapConnectionSelectorInfo ModuleInfoB;
    
    const FRandomStream Random(InSeed);
    FDungeonThemeEngineEventHandlers ThemeEventHandlers;
    ThemeEventHandlers.PerformSelectionLogic = [this, InDungeon, &Random, OwningModuleId, OtherModuleId](const TArray<UDungeonSelectorLogic*>& SelectionLogics, const FDAMarkerInfo& InMarkerInfo) {
        for (UDungeonSelectorLogic* SelectionLogic : SelectionLogics) {
            if (USnapConnectionSelectorLogic* SnapConnectionSelectorLogic = Cast<USnapConnectionSelectorLogic>(SelectionLogic)) {
                if (SnapConnectionSelectorLogic->InvokeSelectNode(InDungeon, OwningModuleId, OtherModuleId, Random, InMarkerInfo.Transform)) {
                    return true;
                }
            }
        }
...
void ASnapConnectionActor::BuildImplDeprecated(ULevel* HostLevel) {
    USnapConnectionInfo* ConnectionInfo = ConnectionComponent->ConnectionInfo;
    const ESnapConnectionState ConnectionState = ConnectionComponent->ConnectionState;
...
void ASnapConnectionActor::BuildConnectionInstance(ADungeon* InDungeon, const FGuid& OwningModuleId, const FGuid& OtherModuleId, ULevel* InHostLevel) {
    ...
    if (ConnectionInfo->Version == static_cast<int32>(ESnapConnectionInfoVersion::LatestVersion)) {
        ...
        BuildImpl(Seed, InDungeon, OwningModuleId, OtherModuleId, InHostLevel);
    }
    else {
