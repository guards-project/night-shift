...
void FSnapStreamingChunkHandlerBase::Internal_SpawnChunkConnections(const FGuid& ChunkID, TArray<FSnapConnectionInstance>& Connections,
    ...
    for (ASnapConnectionActor* ConnectionActor : ConnectionActors) {
        const FGuid ConnectionId = ConnectionActor->GetConnectionId();
        FSnapConnectionInstance* ConnectionData{};
        FGuid ThisModuleId, OtherModuleId;
        for (FSnapConnectionInstance& ConnectionEntry : Connections) {
        ...
        if (ConnectionData) {
            ...
            if (!ConnectionData->bHasSpawnedDoorActor) {
                ...
                ConnectionActor->BuildConnectionInstance(Dungeon.Get(), ThisModuleId, OtherModuleId, DoorLevel);
            ...
            ConnectionActor->BuildConnectionInstance(Dungeon.Get(), ChunkID, {}, WallLevel);
        }
    }
}
...
namespace {
    UDungeonStreamingChunk* GenerateLevelStreamingChunkRecursive(
        ...
        // Generate the level streaming object
        {
            ...
            bool bSuccess = false;
            Chunk->CreateLevelStreaming(World, ModuleLevelAsset, Node->NetworkId, Node->WorldTransform, bSuccess);
        }
