    ...
    for (int32 ModuleIdx : ModuleIndices) {
        ...
        for (int32 AsmIdx : ShuffledAsmIndices) {
            ...
            if (ModuleAssembly.CanFit(InAssembly, DoorIndices)) {
                ...
                if (bDoorCategoriesValid) {
                    // Allow rule resolver blueprints to filter this out
                    bool bAllResolveRulesValid = true;
                    if (Settings.ModuleResolveRules.Num() > 0) {
                        const int32 RotationIndex = AsmIdx;
                        FName Category = ModuleItem->GetCategory();
                        TArray<FName> Tags = ModuleItem->GetTags();
                        TArray<UFlowAbstractNode*> ExistingNodes;
                        for (const auto& Entry : InResolveState.ModuleNodesById) {
                            if (UFlowAbstractNode* ExistingNode = InResolveState.GraphQuery.GetNode(Entry.Key)) {
                                ExistingNodes.Add(ExistingNode);
                            }
                        }

                        for (TObjectPtr<USnapGridFlowModuleSelectionRule> SelectionRule : Settings.ModuleResolveRules) {
                            if (SelectionRule) {
                                if (!SelectionRule->CanSelect(InNode, ExistingNodes, Category, Tags, RotationIndex)) {
                                    bAllResolveRulesValid = false;
                                    break;
                                }
                            }
                        }
...
bool USnapGridFlowModuleSelectionRule::CanSelect_Implementation(UFlowAbstractNode* Node, const TArray<UFlowAbstractNode*>& ExistingNodes, const FName& Category, const TArray<FName>& Tags, int32 RotationIndex) {
    return true;
}
