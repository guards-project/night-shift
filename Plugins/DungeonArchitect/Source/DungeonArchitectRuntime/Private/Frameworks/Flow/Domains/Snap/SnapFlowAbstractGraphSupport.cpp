				...
				TArray<FFlowAGPathNodeGroup>& OutGroups) const
{
	...
	const TSet<FName> AllowedCategories = TSet<FName>(NodeCategorySelector->GetCategoriesAtNode(InPathIndex, InPathLength, NodeCoords.Array()));
...
TArray<FName> FSGFNodeCategorySelector::GetCategoriesAtNode(int32 PathIndex, int32 PathLength, const TArray<FVector>& NodeCoords) const {
	...
	if (ModuleCategoryOverrideMethod == ESnapFlowAGTaskModuleCategoryOverrideMethod::Blueprint) {
		...
		for (USnapFlowNodeCategorySelectionOverride* SelectionOverride : CategoryOverrideLogic) {
			...
			if (SelectionOverride && SelectionOverride->TryOverrideCategories(PathIndex, PathLength, ModuleCategories, NodeCoords, CategoryOverrides)) {
...
bool FSnapGridFlowAbstractGraphConstraints::IsValid(const FFlowAbstractGraphQuery& InGraphQuery, const UFlowAbstractNode* Node,
	...
	TSet<const UFlowAbstractNode*> AllConnectedNodes(IncomingNodes);
	{
		...
		*/
		TSet<UFlowAbstractLink*> ConnectedLinks;
		ConnectedLinks.Append(Graph->GetIncomingLinkObjects(Node->NodeId));
		ConnectedLinks.Append(Graph->GetOutgoingLinkObjects(Node->NodeId));
		for (const UFlowAbstractLink* ConnectedLink : ConnectedLinks) {
			if (!ConnectedLink || ConnectedLink->Type == EFlowAbstractLinkType::Unconnected) {
				continue;
			}
			if (ConnectedLink->Source == Node->NodeId || ConnectedLink->SourceSubNode == Node->NodeId) {
				if (const UFlowAbstractNode* ConnectedNode = InGraphQuery.GetNode(ConnectedLink->Destination)) {
					AllConnectedNodes.Add(ConnectedNode);
				}
			}
			else if (ConnectedLink->Destination == Node->NodeId || ConnectedLink->DestinationSubNode == Node->NodeId) {
				if (const UFlowAbstractNode* ConnectedNode = InGraphQuery.GetNode(ConnectedLink->Source)) {
					AllConnectedNodes.Add(ConnectedNode);
				}
			}
		} 
	...
	FSGFNodeGroupGenLib::BuildNodeGroup(InGraphQuery, Node, AllConnectedNodes.Array(), Group, ConstraintLinks);
...
bool FSnapGridFlowAbstractGraphConstraints::IsValid(const FFlowAbstractGraphQuery& InGraphQuery, const FFlowAGPathNodeGroup& Group, int32 PathIndex, int32 PathLength,
	...
	TSet<FVector> NodeCoords;
	FFlowAbstractGraphPathUtils::GetNodeCoords(InGraphQuery, Group.GroupNodes, NodeCoords);
	const TArray<FName> AllowedCategories = NodeCategorySelector->GetCategoriesAtNode(PathIndex, PathLength, NodeCoords.Array());
