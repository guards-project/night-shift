...
////////////////////////////// FMGGridSceneCell //////////////////////////////
void FMGGridSceneCell::Add(const FDAMarkerInfo& InMarkerInfo, EMarkerGenGridPatternRuleType InType, EMGMarkerListDuplicateCheckMethod DuplicateCheckMethod) {
	// Make sure we don't have duplicates
	if (TArray<FDAMarkerInfo>* MarkerList = GetMarkerList(InType)) {
		bool bFoundDuplicate = false;
		if (DuplicateCheckMethod != EMGMarkerListDuplicateCheckMethod::None) {
			for (const FDAMarkerInfo& MarkerInfo : *MarkerList) {
				if (MarkerInfo.MarkerName == InMarkerInfo.MarkerName) {
					if (DuplicateCheckMethod == EMGMarkerListDuplicateCheckMethod::CheckLocationOnly) {
						if (MarkerInfo.Transform.TranslationEquals(InMarkerInfo.Transform)) {
							bFoundDuplicate = true;
							break;
						}
					}
					else if (DuplicateCheckMethod == EMGMarkerListDuplicateCheckMethod::CheckLocationAndRotation) {
						if (MarkerInfo.Transform.TranslationEquals(InMarkerInfo.Transform) && MarkerInfo.Transform.RotationEquals(InMarkerInfo.Transform)) {
							bFoundDuplicate = true;
							break;
						}
					}
					else if (DuplicateCheckMethod == EMGMarkerListDuplicateCheckMethod::CheckFullTransform) {
						if (MarkerInfo.Transform.Equals(InMarkerInfo.Transform)) {
							bFoundDuplicate = true;
							break;
						}
					}
	...
	, CellHeight(InCellSize.Z)
{
	...
	for (const FDAMarkerInfo& Marker : InMarkers) {
		...
		if (CellPtr) {
			CellPtr->Add(Marker, CoordType, EMGMarkerListDuplicateCheckMethod::CheckFullTransform);
		}
		check(CellPtr);
	} 
}
