...
FString FSimpleCityRoadBeautifier::GetRoadMarkerName(int x, int y, USimpleCityModel* CityModel, float& OutAngle) {
    ...
    if (MatchesConfig(x, y, CityModel, OutAngle, ConfigRoadX, 8)) return FSimpleCityBuilderMarkers::MarkerRoadX;
    if (MatchesConfig(x, y, CityModel, OutAngle, ConfigRoadT, 6)) return FSimpleCityBuilderMarkers::MarkerRoadT;
    if (MatchesConfig(x, y, CityModel, OutAngle, ConfigRoadCorner, 4)) return FSimpleCityBuilderMarkers::MarkerRoadCorner;
    if (MatchesConfig(x, y, CityModel, OutAngle, ConfigRoadS, 4)) return FSimpleCityBuilderMarkers::MarkerRoadS;
    if (MatchesConfig(x, y, CityModel, OutAngle, ConfigRoadEnd, 2)) return FSimpleCityBuilderMarkers::MarkerRoadEnd;

    return FSimpleCityBuilderMarkers::MarkerRoad;
}
