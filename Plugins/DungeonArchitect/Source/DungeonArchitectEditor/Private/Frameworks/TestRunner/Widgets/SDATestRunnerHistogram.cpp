...
int32 SDATestRunnerHistogram::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry,
    ...
    (
        ...
        /*
        AllottedGeometry.ToPaintGeometry(FVector2f(0, 0),
    ...
    for (int32 Index = 0; Index < NumBars; ++Index) {
        ...
        FSlateDrawElement::MakeBox(
            OutDrawElements,
            LayerId,
            AllottedGeometry.ToPaintGeometry(FVector2f(CountX * BarWidth, SizeY), FSlateLayoutTransform(FVector2f(MarkerPosX, AllottedGeometry.GetLocalSize().Y - SizeY - LabelBuffer))),
            /*
            AllottedGeometry.ToPaintGeometry(
