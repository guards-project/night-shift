...
void FDungeonArchitectStyle::FStyle::Initialize() {
    ...
    // Dungeon Actor Styles
    {
        Set( "DungeonActor.BuilderClass.Title", FTextBlockStyle(NormalText)
            .SetFont( DEFAULT_FONT( "BoldCondensed", 20 ) )
            .SetColorAndOpacity( FLinearColor(1, 1, 1, 0.9) )
            .SetShadowOffset( FVector2D::One() )
        );

        Set("DungeonActor.BuilderClass.ItemBorder", new BOX_BRUSH("Common/GroupBorder", FMargin(4.0f / 16.0f)));
    }
    ...
    // Launchpad
    {
        Set("DungeonArchitect.LaunchPad.IconSideBar",
            new PLUGIN_IMAGE_BRUSH("Icons/LaunchPad/da_logo_sidebar_256", Icon128x128));
    }
    
    // Theme Editor
    {
        Set("DAMarkerGen.Build", new PLUGIN_IMAGE_BRUSH("Icons/Common/icon_compile_40x", Icon40x40));
        Set("DAThemeEd.Build", new PLUGIN_IMAGE_BRUSH("Icons/Common/icon_compile_40x", Icon40x40));
        
    }

    // Support Buttons
    {
        Set("DungeonArchitect.OpenSupportForums", new PLUGIN_IMAGE_BRUSH("Icons/Common/icon_help_forum_40x", Icon40x40));
        Set("DungeonArchitect.OpenSupportDiscord", new PLUGIN_IMAGE_BRUSH("Icons/Common/icon_help_bubble_40x", Icon40x40));
        Set("DungeonArchitect.OpenDocumentationWebsite", new PLUGIN_IMAGE_BRUSH("Icons/Common/icon_help_book_40x", Icon40x40));
    }
