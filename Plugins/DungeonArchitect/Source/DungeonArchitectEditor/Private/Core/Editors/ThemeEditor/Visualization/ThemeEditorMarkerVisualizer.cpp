...
void AThemeEditorMarkerVisualizer::BuildVisualization(UDungeonConfig* InConfig, UDungeonModel* InModel, UDungeonBuilder* InBuilder, const TSet<FName>& MarkersToVisualize) const {
	...
	for (const FName& MarkerToVisualize : MarkersToVisualize) {
		const FDAMarkerVisMeshBuilderPtr* MeshBuilderPtr = MeshBuilders.Find(MarkerToVisualize);
		...
		if (Vertices.Num() > 0) {
			...
			MeshComponent->CreateMeshSection(LODIndex, SectionIndex, Vertices, Indices, PT_TriangleList, false);

			if (UMaterialInterface* Material = MeshBuilder->GetMaterial()) {
