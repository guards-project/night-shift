...
void SThemePreviewViewport::PerformMeshRebuild() {
    ...
    DungeonBuilder->ClearMarkerList();
    DungeonBuilder->EmitDungeonMarkers();

    // Emit custom markers in user-created blueprints
    ensure(ObjectToEdit);
    DungeonBuilder->EmitProceduralMarkers(ObjectToEdit->PreviewViewportProperties->ProceduralMarkerEmitters);
    ...
    const TArray<UDungeonMarkerEmitter*> PreviewMarkerEmitters = ObjectToEdit->PreviewViewportProperties->MarkerEmitters;
    const TArray<FClusterThemeInfo> EmptyClusteredThemes; // No clustered themes shown in the theme editor viewport
    DungeonBuilder->ApplyDungeonTheme(Themes, EmptyClusteredThemes, PreviewMarkerEmitters, SceneProvider, PreviewScene->GetWorld());
}
...
void SThemePreviewViewport::InitializeCamera(const UWorld* InWorld) const {
    ...
    for (TActorIterator<AActor> It(InWorld); It; ++It) {
        if (const AActor* Actor = *It) {
            if (Actor->IsLevelBoundsRelevant()) {
                ...
                DungeonBounds += FBox(BoundsOrigin - BoundsExtent, BoundsOrigin + BoundsExtent);
                bInitialized = true;
            }
        }
    }
    ...
    if (bInitialized) {
        constexpr float DungeonBoundsContractionMultiplier = 0.5f;
        const FVector Center = DungeonBounds.GetCenter();
        const FVector Extent = DungeonBounds.GetExtent() * DungeonBoundsContractionMultiplier;
        const FBox SmallerDungeonBounds(Center - Extent, Center + Extent);
        EditorViewportClient->FocusViewportOnBox(SmallerDungeonBounds);
    }
}
