...
void FThemeGraphAppMode::HandleAssetsDropped(const FDragDropEvent& InEvent, TArrayView<FAssetData> InAssets) const {
	if (GraphEditor.IsValid()) {
		if (UEdGraph_DungeonProp* ThemeGraph = Cast<UEdGraph_DungeonProp>(GraphEditor->GetCurrentGraph())) {
					...
					if (PreviewViewport.IsValid()) {
						...
						if (PreviewScene.IsValid()) {
							if (const UWorld* PreviewWorld = PreviewScene->GetWorld()) {
								PreviewLevel = PreviewWorld->PersistentLevel;
							}
						}
					}
				}
			...
			auto CreateBasicAssetNodes = [this, ThemeGraph, &IncrementOffset, PreviewLevel](TArrayView<FAssetData> Assets, FVector2D& InOutOffset) {
				for (const FAssetData& Asset : Assets) {
					if (UObject* AssetObject = Asset.GetAsset()) {
						const FVector2D GridLocation = GetAssetDropGridLocation() + InOutOffset;
						ThemeGraph->CreateNewNode(AssetObject, PreviewLevel, GridLocation);
			...
			if (MultiNodeItemCount > 1) {
				...
				UEdGraphNode_DungeonMeshList* MultiMeshNode = ThemeGraph->CreateNewNode<UEdGraphNode_DungeonMeshList>(NodeLocation);
...
bool FThemeGraphAppMode::AreAssetsAcceptableForDrop(TArrayView<FAssetData> InAssets, FText& OutReason) const {
...
void FThemeGraphAppMode::CreateGraphEditorWidget(UEdGraph* InGraph) {
	...
	AssetDropTarget = SNew(SThemeEditorDropTarget)
		.OnAreAssetsAcceptableForDropWithReason(this, &FThemeGraphAppMode::AreAssetsAcceptableForDrop)
...
void FThemeGraphAppMode::BindCommands(TSharedRef<FUICommandList> ToolkitCommands) {
	...
	ToolkitCommands->MapAction(
		Commands.Build,
		FExecuteAction::CreateSP(this, &FThemeGraphAppMode::HandleGraphChanged));
}
