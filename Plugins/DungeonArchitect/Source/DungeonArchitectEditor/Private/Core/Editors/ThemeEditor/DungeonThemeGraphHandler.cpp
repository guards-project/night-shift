...
////////////////////////////////////// FDungeonArchitectThemeGraphHandler //////////////////////////////////////

void FDungeonArchitectThemeGraphHandler::Initialize(TSharedPtr<SGraphEditor> InGraphEditor, TSharedPtr<IDetailsView> InPropertyEditor,
                                                                       UEdGraph* InThemeGraph, UObject* InEditorProperties) {
    GraphEditorPtr = InGraphEditor;
    PropertyEditorPtr = InPropertyEditor;
    ThemeGraphPtr = InThemeGraph;
    EditorPropertiesPtr = InEditorProperties;
}
...
void FDungeonArchitectThemeGraphHandler::Bind() {
    ...
    GraphEvents.OnSelectionChanged = SGraphEditor::FOnSelectionChanged::CreateSP(this, &FDungeonArchitectThemeGraphHandler::OnSelectedNodesChanged);
    GraphEvents.OnNodeDoubleClicked = FSingleNodeEvent::CreateSP(this, &FDungeonArchitectThemeGraphHandler::OnNodeDoubleClicked);
}
...
void FDungeonArchitectThemeGraphHandler::DeleteSelectedNodes() {
    const TSharedPtr<SGraphEditor> GraphEditor = GetGraphEditor();
    if (GraphEditor.IsValid()) {
...
void FDungeonArchitectThemeGraphHandler::PasteNodesHere(const FVector2D& Location) {
    ...
    UEdGraph* ThemeGraph = GetThemeGraph();
    ...
    FEdGraphUtilities::ImportNodesFromText(ThemeGraph, TextToImport, /*out*/ PastedNodes);
...
bool FDungeonArchitectThemeGraphHandler::CanPasteNodes() const {
    ...
    return FEdGraphUtilities::CanImportNodesFromText(ThemeGraph, ClipboardContent);
}
...
void FDungeonArchitectThemeGraphHandler::OnSelectedNodesChanged(const TSet<class UObject*>& NewSelection) {
    if (NewSelection.Num() > 0) {
        ...
        ShowDefaultEditorProperties();
    }

    SelectedNodesChanged.ExecuteIfBound(NewSelection);
}

void FDungeonArchitectThemeGraphHandler::OnNodeDoubleClicked(UEdGraphNode* Node) {
    NodeDoubleClicked.ExecuteIfBound(Node);
}

TSharedPtr<SGraphEditor> FDungeonArchitectThemeGraphHandler::GetGraphEditor() const {
    return GraphEditorPtr.Pin();
}

UEdGraph* FDungeonArchitectThemeGraphHandler::GetThemeGraph() const {
    return ThemeGraphPtr.Get();
}

void FDungeonArchitectThemeGraphHandler::ShowProperties(UObject* InObject) const {
    TSharedPtr<IDetailsView> PropertyEditor = PropertyEditorPtr.Pin();
    if (PropertyEditor.IsValid()) {
        PropertyEditor->SetObject(InObject);
    }
}

void FDungeonArchitectThemeGraphHandler::ShowProperties(const TArray<UObject*>& InObjects) const {
    TSharedPtr<IDetailsView> PropertyEditor = PropertyEditorPtr.Pin();
    if (PropertyEditor.IsValid()) {
