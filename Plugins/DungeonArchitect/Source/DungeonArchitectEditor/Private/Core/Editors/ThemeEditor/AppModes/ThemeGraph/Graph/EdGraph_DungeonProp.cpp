...
#include "Core/Editors/ThemeEditor/AppModes/ThemeGraph/Graph/EdGraphNode_DungeonParticleSystem.h"
#include "Core/Editors/ThemeEditor/AppModes/ThemeGraph/Graph/EdGraphNode_DungeonPointLight.h"
...
void UEdGraph_DungeonProp::RebuildGraph(UObject* Owner, TArray<FPropTypeData>& OutProps, TArray<FDungeonGraphBuildError>& OutErrors) const {
    ...
    for (UEdGraphNode_DungeonMarker* MarkerNode : MarkerNodes) {
        ...
        for (UEdGraphNode_DungeonActorBase* ActorNode : ActorNodes) {
            ...
            CloneUObjectArray(Owner, ActorNode->SelectionLogics, Prop.SelectionLogics);
            ...
            CloneUObjectArray(Owner, ActorNode->TransformLogics, Prop.TransformLogics);
            Prop.bUseTransformLogic = ActorNode->bUseTransformLogic;

            CloneUObjectArray(Owner, ActorNode->ProceduralTransformLogics, Prop.ProceduralTransformLogics);
            Prop.bUseProceduralTransformLogic = ActorNode->bUseProceduralTransformLogic;

            // Clone the spawn logic instance
            CloneUObjectArray(Owner, ActorNode->SpawnLogics, Prop.SpawnLogics);
...
UEdGraphNode_DungeonBase* UEdGraph_DungeonProp::CreateNewNode(UObject* AssetObject, ULevel* InLevel, const FVector2D& Location) {
    ...
    if (AssetClass->IsChildOf<UStaticMesh>()) {
        ...
        if (UActorFactory* ActorFactory = FActorFactoryAssetProxy::GetFactoryForAsset(AssetData)) {
            ...
            if (ActorFactory->CanCreateActorFrom(AssetData, ErrorMessage)) {
                if (AActor* SpawnedActor = ActorFactory->CreateActor(AssetObject, InLevel, FTransform::Identity)) {
                    // Create an actor node
                    UEdGraphNode_DungeonActorTemplate* ActorNode = CreateNewNode<UEdGraphNode_DungeonActorTemplate>(Location);
                    ActorNode->SetTemplateClass(SpawnedActor->GetClass(), SpawnedActor);
