...
void FSnapMapEditorToolbar::AddFlowVisualizerToolbar(TSharedPtr<FExtender> Extender) {
    check(FlowEditor.IsValid());
    TSharedPtr<FSnapMapEditor> FlowEditorPtr = FlowEditor.Pin();
    ...
    ToolbarExtender->AddToolBarExtension("Asset", EExtensionHook::After, FlowEditorPtr->GetToolkitCommands(),
                                         FToolBarExtensionDelegate::CreateSP(
                                             this, &FSnapMapEditorToolbar::FillFlowVisualizerToolbar));
    FlowEditorPtr->AddToolbarExtender(ToolbarExtender);
}
...
void FSnapMapEditorToolbar::FillFlowDesignerToolbar(FToolBarBuilder& ToolbarBuilder) {
    check(FlowEditor.IsValid());
    const TSharedPtr<FSnapMapEditor> FlowEditorPtr = FlowEditor.Pin();

    if (FlowEditorPtr.IsValid() && FlowEditorPtr->GetCurrentMode() == FSnapMapEditor::GraphEditorModeID) {
        ...
        FillSupportToolItems(ToolbarBuilder);
    }
}

void FSnapMapEditorToolbar::FillFlowVisualizerToolbar(FToolBarBuilder& ToolbarBuilder) {
    check(FlowEditor.IsValid());
    const TSharedPtr<FSnapMapEditor> FlowEditorPtr = FlowEditor.Pin();

    if (FlowEditorPtr.IsValid() && FlowEditorPtr->GetCurrentMode() == FSnapMapEditor::VisualizationModeID) {
...
void FSnapMapEditorToolbar::FillFlowDebugToolbar(FToolBarBuilder& ToolbarBuilder) {
    ...
    if (FlowEditorPtr.IsValid() && FlowEditorPtr->GetCurrentMode() == FSnapMapEditor::DebugModeID) {
        ToolbarBuilder.BeginSection("Flow Control");
        {
...
void FSnapMapEditorToolbar::FillSupportToolItems(FToolBarBuilder& ToolbarBuilder) {
